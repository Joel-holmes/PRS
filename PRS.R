...
v <- c(228,230,226,225,229,227)
predict_response(glength_model, terms = "length [v]")
...
#Rerun and have a look but probably won't include
glength_ts_model <- glmer(binomial_slope~length*tsrange+(1|species), data=test_subset, family = binomial("logit") )
summary(glength_ts_model)
Anova(glength_ts_model)
rm(glength_ts_model)
...
#---------Section 2.3: standardised abundance model -- CHECK IF INCLUDED BEFORE GITHUBBING

zscore_model <- lmer(z_value~year+(1|species), data = shark_data)
summary(zscore_model)

plot_model(zscore_model, type = "pred", terms = c("year"))


#poly model to show uplift
zscore_model_poly <- lmer(z_value ~ poly(year, 2) + (1|species), data = shark_cut)
summary(zscore_model_poly)

plot_model(zscore_model_poly, type = "pred", terms = c("year[all]"), color = "darkblue")+
  theme_classic()

#clean house
rm(zscore_model)
rm(zscore_model_poly)

#---------Section 2.4: species trends since 1990s which have uplift

#Make loop to look at each species between 2000-2020

uplift_subset <- shark_data %>% filter(year>1989)
...
for (species_name in unique(uplift_subset$species)) {
  ...
  if (nrow(uplift_species) < 15) {
    next  
  }
  # Fit a linear model 
  model <- lm(z_value ~ year, data = uplift_species)
  
  # Extract relevant statistics (slope, p-value, R-squared)
  slope <- coef(model)["year"]
  p_value <- summary(model)$coefficients[2, 4]
  r_squared <- summary(model)$r.squared
  std_error <- summary(model)$coefficients[2, 2]
...
#Only include next section if figure 4 is included

additional_info <- shark_data %>% group_by(species,order,superorder, common_names) %>% summarise()

uplift_results <- left_join(uplift_results,additional_info, by = "species")

uplift_results_order <- uplift_results %>%
  group_by(order,superorder) %>%
  summarise(
    mean_slope = mean(slope, na.rm = TRUE),
    n = n(),
    std_error = sd(slope, na.rm = TRUE) / sqrt(n()))
...
#custom colour palette using package colorspace
custom_palette <- function(n) {diverging_hcl(
  n = n,            
  h = c(290, 155),    
  c = c(300, 85),   
  l = c(50, 90)      
...
custom_colours <- custom_palette(5)
...
slope_map <- basemap( limits = c(-180, 180, -70, 80), bathymetry = TRUE, bathy.style = "rcb") + 
  geom_point(data = test_subset, aes(x = long, y = lat, size = abs(slope), color = ifelse(slope < 0, "Negative", "Positive")), alpha = 0.6) +
  scale_size_continuous(range = c(2, 25), limits = c(0, 0.45),name = "Abundance Trend") +
  scale_color_manual(
    values = c("Negative" = "violet", "Positive" = "turquoise"),
    name = "Trend direction")+
  labs(title = "", x = "", y = "") +
  theme_classic()+
  guides(fill= "none", size = guide_legend(override.aes = list(color = "turquoise", alpha = 1)), color = "none")+
  theme(axis.text.x = element_text(size = 16, family = "Arial"),
        axis.text.y = element_text(size = 16, family = "Arial"),
        legend.text = element_text(size = 12, family = "Arial"),
        legend.title = element_text(size = 14, family = "Arial"),
...
ggsave("fullglobeturquoise.png", plot = slope_map, width = 20, height = 10, dpi = 300)
...
early_slope_map <- basemap(
  ...
  geom_point(data = early_map, aes(
    x = long,
    y = lat,
    size = abs(early_slope),  
    color = early_slope), 
    alpha = 0.4) +
  scale_size_continuous(range = c(2, 15), limits = c(0, 0.45), name = "Slope Value") +
  scale_color_gradientn(colors = custom_colours,limits = c(-0.45, 0.45),name = "Slope Value") +
  labs(title = "", x = "", y = "") +
  theme_classic()+
  guides(fill= "none", size = guide_legend(override.aes = list(color = "white")),
...
early_late <- (early_slope_map/late_slope_map) 

ggsave("early_late.png", plot = early_late, width = 10, height = 9, dpi = 300)
...
ggsave("sliding_window2.png", plot = sliding_window, width = 10, height = 9, dpi = 300)
...
#---------Section 3.4: Figure 4? - species and order uplift since 1990s (potentially sj of dist2land too)

#barplot showing order trends since 90s, maybe box and whisker instead
additional_info <- shark_data %>% group_by(species,order,superorder, common_names) %>% summarise()
...
order_uplift <- ggplot(uplift_results_order, aes(x = reorder(order, mean_slope), y = mean_slope, fill = superorder)) +
  geom_bar(stat = "identity", width = 0.5) +
  geom_errorbar(aes(ymin = mean_slope - std_error, ymax = mean_slope + std_error),
  ...
  ggsave("orderuplift.png", plot = order_uplift, width = 10, height = 9, dpi = 300)